...
def training_with_reduce_3dgs():
    """
    使用reduce_3dgs方法的训练函数
    """
    ...
    # 启用reduce_3dgs (这应该通过配置文件设置，这里为演示直接设置)
    gaussians.use_reduce_3dgs = True
    gaussians.reduction_ratio = 0.1  # 保留10%的高斯
    ...
    print(f'Starting training with reduce_3dgs method from iteration {start_iter}')
    print(f'Reduction ratio: {gaussians.reduction_ratio}')
    ...
    for iteration in range(start_iter, training_args.iterations + 1):
        ...
        # reduce_3dgs regularization loss
        if gaussians.use_reduce_3dgs and iteration > optim_args.densify_until_iter:
            # 添加额外的正则化损失来鼓励紧凑表示
            opacity = gaussians.get_opacity
            scale_reg_loss = torch.mean(torch.sum(gaussians.get_scaling, dim=1))  # 鼓励小尺度
            opacity_reg_loss = -torch.mean(opacity * torch.log(opacity + 1e-8))  # 鼓励明确的不透明度
            reduce_reg_loss = 0.001 * scale_reg_loss + 0.001 * opacity_reg_loss
            scalar_dict['reduce_reg_loss'] = reduce_reg_loss.item()
            loss += reduce_reg_loss
            ...
            if iteration < optim_args.densify_until_iter:
                ...
                if iteration > optim_args.densify_from_iter:
                    if iteration % optim_args.densification_interval == 0:
                        ...
                        # 打印reduce_3dgs的额外信息
                        if gaussians.use_reduce_3dgs:
                            print(f"[ITER {iteration}] After reduce_3dgs: {gaussians.get_xyz.shape[0]} gaussians")
    ...
    print(f"\nTraining completed with reduce_3dgs method!")
...
def prepare_output_and_logger():
    ...
    with open(os.path.join(cfg.model_path, "cfg_args"), 'w') as cfg_log_f:
        ...
        viewer_arg['use_reduce_3dgs'] = True  # 标记使用了reduce_3dgs
...
if __name__ == "__main__":
    print("Optimizing with reduce_3dgs method: " + cfg.model_path)
    ...
    training_with_reduce_3dgs()

    # All done
    print("\nTraining with reduce_3dgs complete.")